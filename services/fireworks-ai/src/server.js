...
// ---------------------------------------------------------------------------
// parlel/fireworks-ai — a tiny, dependency-free fake of the Fireworks AI API.
//
// Fireworks AI is OpenAI-compatible and serves its API under /inference/v1.
// Speaks the wire protocol used by the `openai` SDK (pointed at Fireworks) so
// application code and AI agents can run against it at zero cost. All output is
// DETERMINISTIC (hash-derived). Supports SSE streaming. State is in-memory.
// ---------------------------------------------------------------------------
...
function deterministicText(prompt) {
  const words = [
    "Fireworks", "AI", "runs", "fast", "open", "models", "and", "returns",
...
export class FireworksAiServer {
  constructor(port = 4864, options = {}) {
    this.port = port;
  ...
  async handle(req, res) {
    ...
    res.setHeader("server", "parlel-fireworks");
    ...
    // Fireworks routes are under /inference/v1
    if (!(parts[0] === "inference" && parts[1] === "v1")) {
      return this.send(res, 404, oaiError("Unknown endpoint"));
    }
    const route = parts.slice(2);

    if (!this.isAuthorized(req)) {
      return this.send(res, 401, oaiError("Invalid API key", "authentication_error", "invalid_api_key"));
    }

    if (req.method === "GET" && route[0] === "models" && route.length === 1) {
      return this.send(res, 200, {
        object: "list",
        data: MODELS.map((id) => ({ id, object: "model", created: 1700000000, owned_by: "fireworks" })),
      });
    }
  ...
  completions(res, body) {
    if (!isPlainObject(body) || body.prompt === undefined) {
      return this.send(res, 400, oaiError("prompt is a required field"));
    }
    if (typeof body.model !== "string" || !body.model) {
      return this.send(res, 400, oaiError("model is a required field"));
    }
  ...
  root() {
    return { name: "fireworks-ai", version: "1", protocol: "fireworks-v1", documentation: "/docs/fireworks-ai.md" };
  }
